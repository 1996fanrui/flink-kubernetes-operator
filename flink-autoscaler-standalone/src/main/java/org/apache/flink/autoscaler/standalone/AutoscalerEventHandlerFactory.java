...
import static org.apache.flink.autoscaler.standalone.config.AutoscalerStandaloneOptions.EVENT_HANDLER_JDBC_PASSWORD_ENV_VARIABLE;
import static org.apache.flink.autoscaler.standalone.config.AutoscalerStandaloneOptions.EVENT_HANDLER_JDBC_URL;
import static org.apache.flink.autoscaler.standalone.config.AutoscalerStandaloneOptions.EVENT_HANDLER_JDBC_USERNAME;
...
public class AutoscalerEventHandlerFactory {
    ...
    private static <KEY, Context extends JobAutoScalerContext<KEY>>
            AutoScalerEventHandler<KEY, Context> createJdbcEventHandler(Configuration conf)
                    throws Exception {
        final var jdbcUrl = conf.get(EVENT_HANDLER_JDBC_URL);
        checkArgument(
                jdbcUrl != null,
                "%s is required for jdbc event handler.",
                EVENT_HANDLER_JDBC_URL.key());
        var user = conf.get(EVENT_HANDLER_JDBC_USERNAME);
        var password = System.getenv().get(conf.get(EVENT_HANDLER_JDBC_PASSWORD_ENV_VARIABLE));
